...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    if (myToken == 'jwt expired') {
      // Access Token 디코드
      const userInfo = jwt.decode(tokenValue, process.env.TOKEN_KEY)
      ...
      Users.findOne({ where: { userId } }).then((u) => {
        ...
        if (myRefreshToken == 'jwt expired') {
          ...
          // Access Token 발급
          const myNewToken = jwt.sign({ userId }, process.env.TOKEN_KEY, { expiresIn: '15m' })
        }
        res.send({ msg: 'new Access Token', myNewToken: `Bearer ${myNewToken}` })
      })
    } else {
